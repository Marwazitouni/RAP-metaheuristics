...
def read_excel(path,precision):
    ...
    system_df.Probability = system_df.Probability.apply(lambda x: round(x,4))
    subsystems = []
    for subsystem, new_df in system_df.groupby(level=0):
        nb_units = 0
        components = []
        for unit, unit_df in new_df.groupby(level=1):
            nb_state = len(unit_df)
            states = [tuple(x) for x in unit_df[["Probability","Capacity"]].to_records(index=False)]
